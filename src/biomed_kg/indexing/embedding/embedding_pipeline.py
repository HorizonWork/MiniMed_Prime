...
from biomed_kg.common.ids import make_vector_id
from biomed_kg.kg_build.graph_projector import batch_iter


class EmbeddingPipeline:
    def __init__(self, lakehouse, milvus, concept_embedder=None, chunk_embedder=None, assertion_embedder=None):
    ...
    def embed_chunks(self, graph_version: str) -> None:
        ...
        for batch in batch_iter(chunks, 128):
            ...
            for chunk, vector in zip(batch, vectors):
                records.append({"vector_id": make_vector_id("chunk", chunk["chunk_id"]), "object_type": "chunk", "object_id": chunk["chunk_id"], "doc_id": chunk["doc_id"], "source": chunk.get("source"), "section": chunk.get("section"), "entity_ids": chunk.get("entity_ids", []), "cuis": chunk.get("cuis", []), "embedding_model": self.chunk_embedder.model_name, "embedding_dim": len(vector), "text_hash": chunk.get("text_hash"), "vector": vector, "graph_version": graph_version})
            self.milvus.insert(collection="biomed_chunk_dense_v1", records=records)
