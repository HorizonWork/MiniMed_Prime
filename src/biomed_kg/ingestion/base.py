...
from biomed_kg.common.hashing import hash_bytes
...
class SourceIngestionPipeline:
    ...
    def write_raw(self, raw_file: RawFile) -> None:
        ...
        self.object_store.put(
            ...
            data=raw_file.bytes,
            metadata={"checksum": raw_file.checksum, "source": self.source_name, "release": raw_file.release},
        )

    def write_normalized(self, records: Iterable[NormalizedRecord]) -> None:
        if self.lakehouse:
            self.lakehouse.write(table=f"silver_{self.source_name}_records", records=records, mode="append")
