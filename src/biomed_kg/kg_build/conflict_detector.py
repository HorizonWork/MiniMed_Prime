        ...
        for (subject_id, object_id), group in grouped.items():
            ...
            if conflicts:
                ...
                if self.conflict_repo:
                    self.conflict_repo.write(subject_id=subject_id, object_id=object_id, conflict_predicates=list(predicates), conflict_type="predicate_conflict")

    def detect_predicate_conflicts(self, predicates: set[str]) -> list[str]:
        rules = [("treats", "contraindicated_for"), ("causes", "prevents"), ("increases_risk_of", "decreases_risk_of")]
        return [f"{left}_vs_{right}" for left, right in rules if left in predicates and right in predicates]
