...
class Neo4jGraphProjector:
    ...
    def create_constraints(self) -> None:
        self.neo4j.run("CREATE CONSTRAINT entity_id IF NOT EXISTS FOR (e:Entity) REQUIRE e.entity_id IS UNIQUE")
    ...
    def load_entities(self, graph_version: str) -> None:
        ...
        """
        for batch in batch_iter(self.lakehouse.read("entity", {"graph_version": graph_version, "is_active": True}), 10000):
    ...
    def load_assertion_nodes(self, graph_version: str) -> None:
        ...
        """
        for batch in batch_iter(self.lakehouse.read("assertion", {"graph_version": graph_version, "is_current": True}), 10000):
            self.neo4j.run(query, rows=batch)
