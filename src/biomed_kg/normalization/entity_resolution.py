...
class EntityResolutionPipeline:
    ...
    def find_merge_candidates(self, entity) -> list:
        ...
        candidates += self.terminology_service.lookup_by_term(entity.name, semantic_types=getattr(entity, "semantic_types", None))
        return list({getattr(candidate, "entity_id", str(candidate)): candidate for candidate in candidates}.values())
    ...
    def exact_identifier_match(self, entity, candidate) -> bool:
        return getattr(entity, "source_id", None) in sum(getattr(candidate, "identifiers", {}).values(), [])

    def same_cui_and_compatible_type(self, entity, candidate) -> bool:
        return bool(getattr(entity, "cui", None) and entity.cui == getattr(candidate, "canonical_cui", None))
