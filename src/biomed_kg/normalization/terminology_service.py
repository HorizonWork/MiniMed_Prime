...
def rank_term_candidates(text: str, candidates: list):
    ...
    return sorted(candidates, key=lambda candidate: getattr(candidate, "normalized_text", "") != normalized)
...
class TerminologyService:
    ...
    def lookup_by_term(self, text: str, semantic_types: list[str] | None = None):
        ...
        if semantic_types:
            candidates = [candidate for candidate in candidates if intersects(getattr(candidate, "semantic_types", []), semantic_types)]
