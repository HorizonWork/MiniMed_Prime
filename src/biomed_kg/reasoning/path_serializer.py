...
class PathSerializer:
    def __init__(self, entity_vocab, relation_vocab, task_type_vocab, metapath_vocab, question_encoder, option_encoder):
    ...
    def serialize_path(self, task, path, label: int, graph_version: str):
        ...
        return {"task_id": task.task_id, "question_tokens": self.question_encoder.encode(task.question), "option_tokens": self.option_encoder.encode(task.answer_text), "entity_path": entity_tokens, "relation_path": relation_tokens, "edge_confidences": [step.edge_confidence for step in path.steps], "path_length": len(path.steps), "task_type_id": self.task_type_vocab.get_id(task.task_type), "metapath_id": self.metapath_vocab.get_id(path.metapath_template_id), "label": label, "graph_version": graph_version}
