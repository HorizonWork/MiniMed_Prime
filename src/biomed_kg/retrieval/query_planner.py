...
class RetrievalPlan:
    ...
    def get_entity_by_type(self, entity_type):
        ...
        for entity in self.linked_entities:
            if getattr(entity, "entity_type", None) in wanted or getattr(entity, "semantic_type", None) in wanted:
                return entity
...
class QueryPlanner:
    ...
    def plan(self, user_query: str) -> RetrievalPlan:
        ...
        return RetrievalPlan(user_query, task_type, linked_entities, templates, collect_allowed_predicates(templates), collect_disallowed_predicates(task_type), max((template.max_depth for template in templates), default=1))
