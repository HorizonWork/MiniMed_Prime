...
def trm_collator(batch: list[dict]) -> dict:
    return {
        "question_tokens": pad_sequence([item["question_tokens"] for item in batch], batch_first=True),
        ...
        "edge_confidences": pad_sequence([item["edge_confidences"] for item in batch], batch_first=True).float(),
