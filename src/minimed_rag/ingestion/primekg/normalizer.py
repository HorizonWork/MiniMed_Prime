...
from minimed_rag.common.ids import make_entity_id
...
def normalize_primekg_edge(record, predicate_mapper) -> list[NormalizedRecord]:
    subject = normalize_primekg_node(
        record.source_node_id, record.source_node_name, record.source_node_type
    )
    obj = normalize_primekg_node(
        record.target_node_id, record.target_node_name, record.target_node_type
    )
    predicate = predicate_mapper.map_source_predicate(
    ...
    return [
        ...
        NormalizedRecord(
            "source_assertion",
            {
                ...
                "predicate": predicate,
                "object_source_id": obj["source_id"],
