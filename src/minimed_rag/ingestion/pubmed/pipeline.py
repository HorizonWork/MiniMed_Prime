...
from minimed_rag.ingestion.pubmed.normalizer import (
    extract_pubmed_sections,
    normalize_pubmed_document,
)
from minimed_rag.ingestion.pubmed.xml_parser import parse_pubmed_update_xml, parse_pubmed_xml
...
class PubMedIngestionPipeline(SourceIngestionPipeline):
    ...
    def run_baseline(self, release: str = "local") -> None:
        for xml_file in (
            self.downloader.download_baseline_files(release) if self.downloader else self.download()
        ):
            ...
            for citation in parse_pubmed_xml(xml_file):
                ...
                chunks = (
                    []
                    if self.pubmed_chunker is None
                    else [
                        chunk
                        for section in sections
                        for chunk in self.pubmed_chunker.chunk(doc, [section])
                    ]
                )
                if self.lakehouse:
