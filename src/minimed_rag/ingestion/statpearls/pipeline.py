...
from minimed_rag.ingestion.statpearls.normalizer import normalize_statpearls_chapter
from minimed_rag.ingestion.statpearls.parser import RawStatPearlsChapter, parse_statpearls_jsonl
...
class StatPearlsIngestionPipeline(SourceIngestionPipeline):
    ...
    def parse(self, raw_file):
        return [
            RawRecord("StatPearlsChapter", chapter.__dict__)
            for chapter in parse_statpearls_jsonl(raw_file)
        ]
    ...
    def normalize(self, raw_record):
        ...
        return [
            NormalizedRecord(
                type(item).__name__, item.__dict__ if hasattr(item, "__dict__") else item
            )
            for item in normalize_statpearls_chapter(chapter, self.statpearls_chunker)
        ]
