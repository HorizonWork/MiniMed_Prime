...
class PredicateMapper:
    ...
    def map_source_predicate(
        self, source_system: str, source_predicate: str, subject_type: str, object_type: str
    ) -> str:
        mappings = self.registry.get_source_mappings(source_system, source_predicate)
        valid_mappings = []
        for candidate_predicate in mappings:
            if candidate_predicate in self.registry.all() and self.registry.is_valid_domain_range(
                candidate_predicate, subject_type, object_type
            ):
