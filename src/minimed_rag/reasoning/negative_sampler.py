    ...
    def generate_wrong_answer_paths(self, task, graph_version: str):
        ...
        for wrong_answer in wrong_answers:
            ...
            candidate_paths = self.path_finder.find_candidate_paths(
                task.linked_question_entities, linked_wrong, task.task_type, graph_version
            )
    ...
    def generate_relation_corruption_paths(self, positive_paths):
        ...
        for path in positive_paths:
            ...
            step.predicate = self.predicate_registry.sample_incompatible_predicate(
                self.entity_repo.get_type(step.subject_entity_id),
                self.entity_repo.get_type(step.object_entity_id),
                step.predicate,
            )
