...
class PathFinder:
    ...
    def find_candidate_paths(
        self, question_entities: list, answer_entities: list, task_type: str, graph_version: str
    ) -> list:
        ...
        for question_entity in question_entities:
            for answer_entity in answer_entities:
                for template in templates:
                    all_paths.extend(
                        self.find_paths_by_template(
                            question_entity, answer_entity, template, graph_version
                        )
                    )
        return all_paths
    ...
    def find_paths_by_template(self, start_entity, end_entity, template, graph_version: str):
        ...
        return self.find_constrained_variable_paths(
            start_entity, end_entity, template, graph_version
        )
    ...
    def find_one_hop_paths(self, start, end, template, graph_version: str):
        ...
        """
        return [
            self.row_to_path(row, template)
            for row in self.neo4j.query(
                query,
                {
                    "start_id": start.entity_id,
                    "end_id": end.entity_id,
                    "predicate": step.predicate,
                    "min_conf": template.min_edge_confidence,
