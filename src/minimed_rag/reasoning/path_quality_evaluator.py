...
class PathQualityEvaluator:
    def evaluate(self, paths: list) -> dict:
        ...
        associated = sum(
            ...
            if step.predicate == "associated_with"
        )
        return {
            "path_found_rate": float(total > 0),
            "valid_path_rate": valid / total if total else 0.0,
            "associated_with_overuse_rate": associated / total if total else 0.0,
        }
