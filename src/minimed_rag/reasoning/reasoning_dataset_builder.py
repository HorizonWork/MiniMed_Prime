...
class ReasoningDatasetBuilder:
    ...
    def build(self, graph_version: str, dataset_name: str) -> None:
        ...
        for task in tasks:
            linked_question_entities = self.entity_linker.link_mentions(
                task.question, self.ner.extract(task.question)
            )
            ...
            candidate_paths = self.path_finder.find_candidate_paths(
                linked_question_entities, linked_answer_entities, task.task_type, graph_version
            )
            ...
            examples = self.path_serializer.serialize_examples(
                task, positive_paths, negative_paths, graph_version
            )
